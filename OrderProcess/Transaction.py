...
class Transaction():
    def __init__(self, balance, positions, leverage):
        self.history =[] #closed orders list
        self.balance = balance
        self.positions = positions #open orders list
        self.pending = []
...
class NoLewerage(Transaction):
    ...
    def buy(self, price,candle, transactionHistory,  size = 1.0,takeProfit = 0.0, stopLoss = 0.0):
        if(size*price < self.balance):
                ...
                dict = {'take profit' : takeProfit, 'stop loss' : stopLoss}
                
                pos = Order.Order(size, Order.Direction.BUY, Order.OrderType.MARKET, dict, price= price, candle = candle, )
                self.positions.append(pos)
        else:
                self.balance-= math.floor(self.balance/size) * price

                pos = Order.Order(math.floor(self.balance/size), Order.Direction.BUY, Order.OrderType.MARKET, dict, price= price, candle = candle, )
                self.positions.append(pos)
        return self.balance, self.positions
    ...
    def sell(self, price,candle, transactionHistory, size = 1.0,takeProfit = 0.0, stopLoss = 0.0):
             while size >0.0:
                 if(len(self.positions) > 0):
                     ...
                     if posToClose.size > size:
                         closedPos = copy.deepcopy(posToClose)
                         posToClose.size -= size
                         closedPos.setSize(size)
                         closedPos.setClosePriceAndCandle(price, candle)
                         self.positions.append(posToClose)

                         self.history.append(closedPos)
                         self.balance += size*price
                         size = -1
                     elif  posToClose.size <= size:
                         self.balance += posToClose.size*price
                         posToClose.setClosePriceAndCandle(price, candle)
                         self.history.append(posToClose)
    ...
    def closeAllPositions(self, positions, balance, candle,lastPrice,toClose):
        ...
        if(toClose):
            for pos in self.positions:
                balance += pos.size * lastPrice
                pos.setClosePriceAndCandle(lastPrice, candle)
                self.history.append(pos)
            self.positions = []
        else:
            for pos in self.positions:
                assetsVal += pos.size * lastPrice
...
class Lewerage(Transaction):
    ...
    def buy(self, price, transactionHistory, candle,  size = 1000.0, takeProfit = 0.0, stopLoss = 0.0):
        if(len(self.positions) != 0):
            if(self.positions[0].direction is Order.Direction.SELL):
        ...
        if(size > 0):
            ...
            if(size*price/self.leverage < self.balance):
                ord = Order.Order(size, Order.Direction.BUY, Order.OrderType.MARKET, {'take profit': takeProfit, 'stop loss': stopLoss}, price, candle)
                self.positions.append(ord)
                #self.history.append((candle, size, price))
    ...
    def sell(self, price, transactionHistory, candle, size = 1.0, takeProfit = 0.0, stopLoss =0.0):
        #size *= 1
        if(len(self.positions) != 0):
            if(self.positions[0].direction is Order.Direction.BUY):
        ...
        if size >0:
             if(-size*price/self.leverage < self.balance):
                ...
                Order.Order(size, Order.Direction.SELL, Order.OrderType.MARKET, {'take profit': takeProfit, 'stop loss': stopLoss}, price, candle)
                self.balance -= price*size/self.leverage
            ...
            while(size > 0):
                if(len(self.positions) > 0):
                    ...
                    if posToClose.size > size:
                        posToClose.size -= size
                        self.positions.append(posToClose)
                        self.balance += (size*(price - posToClose.price) + size*posToClose.price/self.leverage)
                        copyPos = copy.deepcopy(posToClose)
                        copyPos.size = size
                        copyPos.closeCandle = candle
                        copyPos.closePrice = price
                        self.history.append(copyPos)
                        size = 0
                    elif  posToClose.size <= size:
                        self.balance += posToClose.size*(price-posToClose.price) + posToClose[0]*posToClose[1]/self.leverage
                        size-=posToClose[0]
                        self.history.append((candle, -posToClose[0], price))
                else:
                    break
